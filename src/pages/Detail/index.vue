				...
				<div class="InfoWrap">
					...
					<div class="choose">
						<div class="chooseArea">
							...
							<dl
								v-for="attrList in spuSaleAttrList"
								:key="attrList.id"
							>
								<dt class="title">
									{{ attrList.saleAttrName }}
								</dt>
								<dd
									changepirce="0"
									:class="{ active: attr.isChecked === '1' }"
									v-for="attr in attrList.spuSaleAttrValueList"
									:key="attr.id"
									@click="
										changeCheck(
											attr,
											attrList.spuSaleAttrValueList
										)
									"
								>
...
<script>
	...
	export default {
		...
		computed: {
			//将 getters 中简化的数据映射到组件
			...mapGetters(["categoryView", "spuSaleAttrList", "skuInfo"]),
			//商品图片列表
			skuImageList() {
				// skuInfo 在页面刚打开时会是一个空对象，空对象 去 . 属性是 undefined
				//undefined 当数组用会导致假报错，为了防止这一情况，我们需要做一个修正
				return this.skuInfo.skuImageList || [];
			},
		},
		methods: {
			//组件中 dispatch 发送请求的方法
			getGoodsDetailInfo() {
				this.$store.dispatch("getGoodsDetailInfo", this.skuId);
			},
			//商品属性点击事件
			changeCheck(attr, attrList) {
				//功能：点击属性切换高亮，接收 被点击属性 和 属性列表 作为参数
				//遍历属性列表将所有属性的高亮取消
			...
			//添加购物车按钮点击事件回调
			async addShopCart() {
				//await 接到的是失败的 promise 时会抛出错误，我们使用 trycatch 进行捕获和处理
				try {
					//使用 await ，用同步代码实现异步效果
					await this.$store.dispatch("addOrUpdateShopCart", {
						skuId: this.skuId,
						skuNum: this.skuNum,
					});
					//当请求成功时， await 会接受到成功的promise 的结果值，代码会继续向下执行
					alert('添加购物车成功！')
					//将商品信息临时存储到 sessioStorage 中，方便添加成功页使用
